...
<div id="wrapper">	
	...
	<div id="content" class="xgrid">
		...
		<div class="x12">
			...
			<h2>发票出库</h2>
			<form action="addivoutin.do"  method="post">
			<div class="form label-inline uniform">
	
				<div class="field"><label for="id">出库时间</label> <input id="id" name="inDate" size="50" type="text" class="medium" disabled="disabled" value="<% out.print(new java.text.SimpleDateFormat("yyyy-MM-dd").format(new Date())); %>" /></div>
				
				<div class="field"><label for="name">出库人</label> <input id="name" name="lname" size="50" type="text" class="medium" disabled="disabled" value="${emp.empName }" />
				                                                   <input  name="eid" type="hidden" value="${emp.id }" /></div>
				
				<div class="field"><label for="name">领用人</label>
                    <select class="medium" name="empID">
                    <c:forEach items="${elist }" var="e">
						<option value="${e.id }">${e.empName }</option>
						</c:forEach>
					</select></div>
					
				<div class="field"><label for="name">发票类型</label> <input id="sNo" name="type" size="50" type="text" class="medium" disabled="disabled" value="填入发票号码后，Ajax查询" />
				...
				<div class="field"><label for="name">发票终止号码</label><input id="endNo" name="endNo" size="50" type="text" class="medium" onblur="checkUsername()"/></div>
				
				<div class="field"><label for="name">发票张数</label> <input id="count" name="counts" size="50" type="text"   value="填入发票号码后，动态显示" /></div>
