...
<jsp:include page="../index.jsp"/>
	...
	<div id="content" class="xgrid">
		...
		<div class="x12">
			...
			<h2>设置片区权限</h2>
			<form action="addEmpAreaPowers.do" method="post">
			<div class="form label-inline uniform">
				...
				<c:forEach items="${emplst }" var="el">
					<input type="hidden" name="empID" value="${el.id}" />
				</c:forEach>
				<c:if test="${emplst==null }">
				...
				<div class="controlset field">
					...
					<div class="controlset-pad"><input type="checkbox"  name="areaID" value="${area.id }"  
					<c:forEach items="${alist }" var="al">
							<c:if test="${al.id==area.id }">checked="checked"</c:if>
					</c:forEach> 
					/></div>
				</div>	
				</c:forEach>
